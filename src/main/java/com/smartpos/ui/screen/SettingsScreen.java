...
public class SettingsScreen {
    ...
    public Parent build() {
        ...
        Label pinHeading = new Label("Credit Authorization PIN");
            pinHeading.setStyle("-fx-font-weight: bold;");

            PasswordField pinField = new PasswordField();
            pinField.setPromptText("4-digit PIN");
            pinField.setText(settings.getOrDefault("userPin", ""));

            Label pinFeedback = new Label();
            pinFeedback.setStyle("-fx-font-size: 11px;");

            Button savePin = new Button("Save PIN");
            savePin.setMaxWidth(Double.MAX_VALUE);

            savePin.setOnAction(e -> {
                String pin = pinField.getText().trim();

                if (!pin.matches("\\d{4}")) {
                    pinFeedback.setText("PIN must contain exactly 4 digits.");
                    pinFeedback.setTextFill(Color.RED);
                    return;
                }

                context.settingsService().set("userPin", pin);

                pinFeedback.setText("Credit authorization PIN saved.");
                pinFeedback.setTextFill(Color.GREEN);
            });
